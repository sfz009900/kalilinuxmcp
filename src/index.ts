...
async function main() {
  try {
    ...
    try {
      ...
      // 显示实时推送状态
      const realtimeStatus = commandExecutor.getRealtimePusherStatus();
      log.info("=== 实时输出查看器状态 ===");
      log.info(`实时推送: ${realtimeStatus.enabled ? '已启用' : '已禁用'}`);
      log.info(`查看器URL: ${realtimeStatus.config.viewerUrl}`);
      log.info("提示: 启动实时查看器后，所有命令输出将实时显示在浏览器中");
